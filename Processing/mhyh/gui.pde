...
public void amplitude_val_change(GTextField source, GEvent event) {
  if (event.toString() == "TextField/Area lost focus") {
    int value = int(source.getText());
    if (side == "A") {
      amplitude_a = value/100;
    } else {
      amplitude_v = value/100;
    }
...
public void amplitude_slider_change(GCustomSlider source, GEvent event) { //_CODE_:custom_slider1:464409:
  //println("amp slider");
  if (side == "A") {
    amplitude_val_a_field.setText(str(source.getValueF()/100));
    amplitude_a = source.getValueF()/100;
  } else { 
    amplitude_val_v_field.setText(str(source.getValueF()/100)); 
    amplitude_v = source.getValueF()/100;
  }
} 
